            ...
            if (is_object($o_el)===false) {
                ...
                if ($o_el->showInEmail() === true) {
                    ...
                    if(empty($s_val)===false){
                        $s_ret.='<tr valign="top" bgcolor="' . $bgCol . '"><td><b>' . strip_tags($o_el->getLabel()) . ':</b></td><td>' . $s_val . '</td></tr>';
                    }
                    $rowCount++;
                }
            }
                ...
                if ($o_el->showInEmail() === true) {
                    ...
                    switch ($elType) {
                        case 'JsonFormBuilder_elementMatrix':
                            ...
                            $c_cnt = 0;
                            foreach ($cols as $column) {
                                $s_val.= strip_tags($column) . '';
                                $c_cnt++;
                            }
                            $s_val.="\r\n";
                            foreach ($rows as $row) {
                                $c_cnt = 0;
                                $s_val.= strip_tags($row) . ' ';
                                foreach ($cols as $column) {
                                    switch ($type) {
                                        case 'text':
                                            $s_val.=strip_tags($this->postVal($elId . '_' . $r_cnt . '_' . $c_cnt));
                                            break;
                                        case 'radio':
                        ...
                        case 'JsonFormBuilder_elementDate':
                            ...
                            break;
                        default:
                            $s_val = strip_tags($this->postVal($o_el->getId()));
                            break;
                    }
                    if(empty($s_val)===false){
